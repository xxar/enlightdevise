...
<head>
    ...
    <script type="text/javascript" src="/skin/ziyong/Scripts/org.1494058893.js" data-main="postMain"></script>
    <script type="text/javascript" src="/skin/ziyong/Scripts/2041.js"></script>
    <script>document.domain = "uemo.net";</script>
    <title><?=$grpagetitle?></title></head>
...
<div id="pageTarget" class="module">
    <div class="module_container">
        <div class="container_target wow">您的位置：
            ...
            <a ><?=$class_r[$grbclassid][classname]?></a>
            <i class="fa fa-angle-right"></i>
            <a ><?=$class_r[$ecms_gr[classid]][classname]?></a></div>
        <div class="clear"></div>
    </div>
</div>
<div class="content">
...
<div id="postContent">
    <div id="postInfo">
        <p class="title"><?=$ecms_gr[title]?></p>
        <p class="subtitle"><?=date('Y-m-d',$ecms_gr[newstime])?></p></div>
    <div class="postbody">
        <?=$ecms_gr[newsinfo]?>
    </div>
</div>
    ...
    <div class="mlist news">
        <div class="content_wrapper">
            <div class="content_list">
                <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('selfinfo',5,0,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
                <div  class="item_block wow first" style="animation-delay:.0s">
