    ...
    <div class="bao back2">
        <div class="wiNer">
            <div class="top">
                <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq("select classid,classname from {$dbtbpre}enewsclass where bclassid=5 ",0,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
<section>
    <div class="bao pad5">
        <div class="wiNer">
            <div class="bt10">
                <b><?=$ecms_gr[title]?></b>
                <span>时间：<?=date('Y-m-d',$ecms_gr[newstime])?>     来源： [!--befrom--]</span>
            </div>
            <?=$ecms_gr[newsinfo]?>
        </div>
    </div>
